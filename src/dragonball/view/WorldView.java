...
public class WorldView extends JPanel implements Serializable {
	...
	public WorldView(Player player, World world){
		...
		try 
		{
			bufferedImage = ImageIO.read(new File("map.jpg"));
		} 
		catch (IOException e) {
			...
			for(int i=0; i<10; i++)
			{
				for (int j=0; j<10; j++)
				{
					if(!((i==0 && j==0)))
					{
						...
						{
							bufferedImageFoe=ImageIO.read(new File("strongFoe.png"));
							cellLabel=new JLabel(new ImageIcon(fitImageToACertainSize(60,60,bufferedImageFoe)));
		...
		submitChoose.setForeground(Color.WHITE);
		...
		assignSA.setForeground(Color.WHITE);
		...
		assignUA.setForeground(Color.WHITE);
		...
		submitAdd.setForeground(Color.WHITE);
		...
		submitReplace.setForeground(Color.WHITE);
		...
		changeFighter.setForeground(Color.WHITE);
		...
		AssignAttacks.setForeground(Color.WHITE);
